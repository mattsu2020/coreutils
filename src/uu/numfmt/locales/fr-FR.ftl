...
numfmt-help-grouping = utiliser le groupement défini par la locale
numfmt-help-unit-separator = insérer SEP entre le nombre et l'unité en sortie, et exiger SEP entre eux en entrée
numfmt-help-invalid = définir le mode d'échec pour les entrées invalides
numfmt-help-debug = afficher des avertissements concernant les entrées invalides
...
numfmt-error-invalid-suffix-detail = suffixe invalide dans l'entrée { $input } : { $suffix }
...
numfmt-error-value-too-large-to-print = valeur trop grande pour être imprimée : { $value } (envisagez d'utiliser --to)
numfmt-error-value-precision-too-large-to-print = valeur/précision trop grande pour être imprimée : { $value_precision } (envisagez d'utiliser --to)
numfmt-error-value-too-large-to-convert = valeur trop grande pour être convertie : { $value }
numfmt-error-value-too-large-to-print-suffix = valeur trop grande pour être imprimée : { $value } (impossible de gérer des valeurs > 999Q)
...
numfmt-debug-field-delimiter-precedence = les délimiteurs de champs ont la priorité sur les séparateurs d'unités
numfmt-debug-header-cli = --header est ignoré avec les paramètres en ligne de commande
numfmt-debug-invalid-inputs = certains nombres de l'entrée n'ont pas pu être convertis
numfmt-debug-format-overrides-padding = --format remplace la valeur fournie à --padding
