...
numfmt-help-grouping = use locale-defined grouping of digits
numfmt-help-unit-separator = insert SEP between number and unit on output, and expect SEP between them on input
...
numfmt-error-invalid-suffix-detail = invalid suffix in input { $input }: { $suffix }
...
numfmt-error-value-too-large-to-print = value too large to be printed: { $value } (consider using --to)
numfmt-error-value-precision-too-large-to-print = value/precision too large to be printed: { $value_precision } (consider using --to)
numfmt-error-value-too-large-to-convert = value too large to be converted: { $value }
numfmt-error-value-too-large-to-print-suffix = value too large to be printed: { $value } (cannot handle values > 999Q)
...
numfmt-debug-field-delimiter-precedence = field delimiters have higher precedence than unit separators
numfmt-debug-header-cli = --header ignored with command-line input
numfmt-debug-invalid-inputs = failed to convert some of the input numbers
numfmt-debug-format-overrides-padding = --format padding overriding --padding
